...
Функция Настройки()
	...
	Настройки.Вставить("КаталогРепозитория", "C:\git\bslpretty\");
...
Функция СпроситьПользователяДаНет(ТекстВопроса)
	...
	ВариантыОтветов.Вставить("Нет", Истина);
	ВариантыОтветов.Вставить("Yes", Истина);
	ВариантыОтветов.Вставить("No", Истина);
	ВариантыОтветов.Вставить("Д", Истина);
	ВариантыОтветов.Вставить("Н", Истина);
	ВариантыОтветов.Вставить("Y", Истина);
	ВариантыОтветов.Вставить("N", Истина);
...
Функция ПолучитьМодифицированныеФайлы()
	...
	КодВозврата = -1;
	ЗапуститьПриложение("cmd /C git diff -U0 > """ + ЛогГит + """", Настройки.КаталогРепозитория, Истина, КодВозврата);
	...
	ЛогГит = ПолучитьИмяВременногоФайла();
	КодВозврата = -1;
	ЗапуститьПриложение("cmd /C git status -s > """ + ЛогГит + """", Настройки.КаталогРепозитория, Истина, КодВозврата);
	Если КодВозврата <> 0 Тогда
...
Для каждого Инф ИЗ МодифицированныеФайлы Цикл
	...
	Попытка
		...
		Иначе
			...
			Для Ц=0 По МассивИсходник.Количество()-1 Цикл
				...
				Если Инф.ИзмененныеСтроки.Найти(ц) = Неопределено Тогда
	...
	Иначе
		...
		ЗапуститьПриложение("cmd /K """ + КомандаDiff + """", Настройки.КаталогРепозитория, Истина);
