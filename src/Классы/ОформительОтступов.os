...
Функция Закрыть() Экспорт
	...
	// Составим карту отступов - массив в котором i-ому элементу соответствует количество отступов в i-ой строки
	КартаОтступов = Новый Массив();
	Для ц = 0 По ТаблицаТокенов[ТаблицаТокенов.Количество() - 1].НомерСтроки Цикл
		КартаОтступов.Добавить(0);
	КонецЦикла;
	...
	Для Индекс = 1 По КоличествоТокенов - 1 Цикл
		...
		Если Индекс > 1 И Токен.НомерСтроки - ТаблицаТокенов[Индекс - 1].НомерСтроки > 0 Тогда
			...
			КартаОтступов[ТокенНачало.НомерСтроки] = КартаОтступов[ТокенНачало.НомерСтроки] + ОтступыЭтойСтроки;
			...
			Для ц = ТокенНачало.НомерСтроки + 1 По Токен.НомерСтроки - 1 Цикл
				Если мПустыеСтрокиСОтступами Тогда
					КартаОтступов[ц] = КартаОтступов[ц] + ОтступыЭтойСтроки + СтекДополнительныхОтступов.Количество();
				КонецЕсли;
			КонецЦикла;
			
			Если мРекурсивныеДополнительныеОтступы Тогда
				КартаОтступов[Токен.НомерСтроки] = СтекДополнительныхОтступов.Количество();
			ИначеЕсли СтекДополнительныхОтступов.Количество() > 0 Тогда
				КартаОтступов[Токен.НомерСтроки] = 1;
			КонецЕсли;
	...
	// Теперь расставим отступы
	ДобавитьЗаменыРасстановкиОтступов(КартаОтступов);
...
Процедура ДобавитьЗаменыРасстановкиОтступов(КартаОтступов)
	
	Для Индекс = 1 По ТаблицаТокенов.Количество() - 1 Цикл
		...
		Для НомерСтроки = Начало.НомерСтроки + 1 По Конец.НомерСтроки Цикл
			...
			СтрокаОтступы = "";
			Для цц = 1 По КартаОтступов[НомерСтроки] Цикл
