...
Функция НастройкиВыполнения()
	...
	Настройки.Вставить("ИмяФайлаСНастройкамиПлагинов", Неопределено);
	...
	Если ФайлНастроек.Существует() Тогда
		...
		Настройки.ИмяФайлаСНастройкамиПлагинов = ПутьКФайлуНастроек;
		
	Иначе
		
		Сообщить("Файл настроек не существует " + ПутьКФайлуНастроек);
		ЗавершитьРаботу(1);
...
Оформитель = Новый ОформительИсходногоКода();
Оформитель.ПрочитатьНастройкиИзФайла(Настройки.ИмяФайлаСНастройкамиПлагинов);
...
Для Каждого Инф Из Настройки.ФайлыДляОформления Цикл
	...
	Попытка
		...
		ИсправленныйИсходник = Оформитель.Оформить(Исходник);
		
		// Применим исправления только к измененным строчкам кода
